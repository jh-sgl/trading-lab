...
@register_model("ReprExtremaDetector")
class ReprExtremaDetectorModel(L.LightningModule):
    ...
    def _step(self, batch, cache: CacheDict | None) -> torch.Tensor | tuple[torch.Tensor, torch.Tensor]:
        ...
        if not self.training:
            start = torch.randint(0, len(output) - 5, (1,))
            for o, p in zip(output[start : start + 5], price_exit_label[start : start + 5]):
