...
@register_callback("ReprExtremaDetectorBacktester")
class ReprExtremaDetectorBacktesterCallback(L.Callback):
    def __init__(
        self,
        ...
        super().__init__()
        self._backtester = ReprExtremaDetectorBacktester(
            tick_slippage_size,
            commission_rate,
            price_multiplier,
            initial_balance,
    ...
    def _create_output_df_from_cache(self, cache: CacheDict, keys: list[DataKey]) -> pd.DataFrame:
    ...
    def on_validation_end(self, trainer: L.Trainer, pl_module: ReprExtremaDetectorModel) -> None:
        if trainer.sanity_checking:
            return

        keys = [DataKey.TIMESTAMP, DataKey.OUTPUT_PRED]

        train_output_df = self._create_output_df_from_cache(pl_module.train_cache, keys)
        val_output_df = self._create_output_df_from_cache(pl_module.val_cache, keys)
        ...
        self._run(trainer, train_output_df, train_dataset_df, False, "TRAIN")
        self._run(trainer, train_output_df, train_dataset_df, True, "TRAIN-COSTLESS")
        self._run(trainer, val_output_df, val_dataset_df, False, "VALID")
        self._run(trainer, val_output_df, val_dataset_df, True, "VALID-COSTLESS")
        self._run(trainer, total_output_df, total_dataset_df, False, "TOTAL")
        self._run(trainer, total_output_df, total_dataset_df, True, "TOTAL-COSTLESS")
    ...
    def _run(
        ...
        backtest_result = self._backtester.run_backtest(backtest_input_df, ignore_cost=ignore_cost)
        stats = self._trading_stats.calc_stats(backtest_result)
        plot_save_fp, backtest_dataframe_fp = self._setup_save(
            trainer.log_dir, save_prefix, trainer.current_epoch, stats
        )
...
class ReprExtremaDetectorBacktester:
    def __init__(
        self,
        tick_slippage_size: float,
        commission_rate: float,
    ...
    def _decide_position(self, output_pred: pd.Series, volatility: pd.Series) -> pd.Series:
        if self._position_strategy == "risk_volatility_adjustment":
            ...
            max_position = self._calc_max_position(np.clip(volatility, a_min=0.1, a_max=10))
            position = np.round(abs(output_pred * max_position).clip(1, float("inf")), 0) * np.sign(output_pred)
    ...
    def _calc_profit(
        ...
        # profit = price_move * position - contract_cost
    ...
    def run_backtest(self, df: pd.DataFrame, ignore_cost: bool = False) -> pd.DataFrame:
        ...
        profit = self._calc_profit(
            ...
            contract_cost,
            apply_price_multiplier=True,
            # position,
            # df[DataKey.PRICE_MOVE],
            # contract_cost,
            # apply_price_multiplier=True,
        )
        backtest_result = self._create_result_dataframe(
            df[DataKey.TIMESTAMP],
            position,
            contract_cost,
            df[DataKey.PRICE_ENTER],
            profit,
            # df.index, position, contract_cost, df[DataKey.PRICE_ENTER], profit
        )
