#!/bin/bash
RUN_NAME_PREFIX="multirun_0b_with_genfactor"
CONFIG_NAME="v1"
INPUT_DIM=20
...
run_job() {
  ...
  while (( attempt <= MAX_RETRIES )); do
    echo
    echo "🚴‍♀️ Running $ta_name with $genfactor (Attempt $attempt)"
    echo
    python -m task.EXP0016_SplitStrategy.train \
      --config-name=${CONFIG_NAME} \
      input_dim=${INPUT_DIM} \
      exp_name=${run_name}/${ta_name}/${genfactor}/${valid_year} \
      ...
      "gen_factorset=[ {name: \"${genfactor}\", args: random }, {name: \"${genfactor}\", args: random }, {name: \"${genfactor}\", args: random }, {name: \"${genfactor}\", args: random } ]" \
...
for genfactor in "${GENFACTOR_LIST[@]}"; do
  for ta_name in "${TA_LIST[@]}"; do
    for ((i=0; i<${#YEAR_LIST[@]} - 5; i++)); do
      ...
      run_job "$ta_name" "$RUN_NAME" "$train_year" "$valid_year" "$test_year" "$genfactor" &
