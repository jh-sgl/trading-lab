...
@register_network("regime_classifier")
class RegimeClassifier(nn.Module):
    def __init__(
        self, regime_lookback_num: int, regime_input_cols_num: int, num_classes: int
    ):
        super().__init__()

        self._repr_mlp = nn.Sequential(
            nn.Flatten(),
            nn.Linear(24 * 16, 32),
            nn.BatchNorm1d(32),
            nn.GELU(),
        ...
        self._regime_col_mlp = nn.Sequential(
            nn.Flatten(),
            nn.Linear(regime_lookback_num * regime_input_cols_num, 64),
            nn.BatchNorm1d(64),
            nn.GELU(),
            nn.Linear(64, 32),
            nn.BatchNorm1d(32),
    ...
    def forward(
        self,
        repr_inputs: torch.Tensor,
        col_inputs: torch.Tensor,
        meta_inputs: torch.Tensor,
    ) -> torch.Tensor:
        # repr_out = self._repr_mlp(repr_inputs)  # (B, 16, 24)
        col_out = self._regime_col_mlp(col_inputs)
        meta_out = self._meta_mlp(meta_inputs)
        combined = torch.cat([col_out, meta_out], dim=1)
        # combined = torch.cat([repr_out, col_out, meta_out], dim=1)
        return self._classifier(combined)
