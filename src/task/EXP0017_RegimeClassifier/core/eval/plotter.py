...
class ResultPlotter:
    ...
    def _draw_balance_subplot(
        self,
        ax: Axes,
        train_date_range: tuple[str, str] | None = None,
        valid_date_range: tuple[str, str] | None = None,
        ...
        ax.axvspan(
            pd.to_datetime(train_date_range[0]),
            pd.to_datetime(train_date_range[1]),
            color="green",
            alpha=0.05,
            label="TRAIN",
        )
        ax.axvspan(
            pd.to_datetime(valid_date_range[0]),
            pd.to_datetime(valid_date_range[1]),
            color="gold",
            alpha=0.05,
            label="VALID",
        )
        ax.axvspan(
            pd.to_datetime(test_date_range[0]),
            pd.to_datetime(test_date_range[1]),
            color="red",
            alpha=0.05,
            label="TEST",
        )
    ...
    def _draw_statistics_subplot(self, ax: Axes, ax_tab: Axes) -> None:
        ...
        _add_table_item(
            table_data,
            self._stats.net_stat.twa_sharpe_ratio,
            "Time-weighted Accumulated Sharpe-Ratio",
            a_unit="",
        )
        _add_table_item(
    ...
    def draw_result(
        ...
        self._draw_balance_subplot(
            ax1, train_date_range, valid_date_range, test_date_range
        )
