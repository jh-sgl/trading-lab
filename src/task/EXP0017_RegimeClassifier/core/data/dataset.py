...
from ...util.expiry_dates import calc_days_to_expiration_KOSPI200
...
class BasicDataset(Dataset):
    def __init__(
        self,
        regime_label_fp: str,
        regime_distance_threshold: float,
        regime_input_cols: list[tuple[str, str]],
        regime_lookback_num: int,
        data_fp: str,
        ta_factorset_fp: str,
        ...
        self._regime_input_cols = regime_input_cols
        self._regime_lookback_num = regime_lookback_num

        df, self._factor_cols, self._regime_label = self._load_data(
            regime_label_fp,
        ...
        cols_to_process = pd.MultiIndex.from_tuples(
            list(self._factor_cols) + self._regime_input_cols
        )
        df = self._drop_unused_rows(df, cols_to_process)
        self._df = self._scale_all_data_by_train_date_range(
            df, cols_to_process, train_date_range
        )
        ...
        self._regime_col_tensor = torch.from_numpy(
            self._df[self._regime_input_cols].to_numpy(dtype=np.float32)
        )
        self._regime_label_map = {
            item: i for i, item in enumerate(self._regime_label.unique())
        }
        ...
        self._regime_train_dataloader_idx, self._regime_valid_dataloader_idx = (
            self._create_regime_dataloader_idx(
                self._df, self._train_date_range, self._valid_date_range
            )
        )
    ...
    def regime_num_classes(self) -> int:
        return len(self._regime_label_map)

    @property
    def regime_train_dataloader_idx(self) -> list[int]:
        return self._regime_train_dataloader_idx

    @property
    def regime_valid_dataloader_idx(self) -> list[int]:
        return self._regime_valid_dataloader_idx
    ...
    def set_learning_stage(
        ...
        if learning_stage not in ["repr", "regime", "signal"]:
            raise ValueError(f"Unknown learning stage: {learning_stage}")

        self._learning_stage = learning_stage

        if repr_model is not None:
            if learning_stage == "regime":
                dataloader_idx = (
                    self._regime_train_dataloader_idx
                    + self._regime_valid_dataloader_idx
                )
                self._regime_dataloader_idx_to_repr_idx = {
                    didx: ridx for ridx, didx in enumerate(dataloader_idx)
                }
            elif learning_stage == "signal":
                dataloader_idx = self._signal_total_dataloader_idx
                self._signal_dataloader_idx_to_repr_idx = {
                    didx: ridx for ridx, didx in enumerate(dataloader_idx)
                }
            else:
                raise ValueError(
                    f"setting {learning_stage} with repr_model not supported"
                )

            with torch.no_grad():
                logging.info(
                    f"Creating Representation for {learning_stage} Training..."
                )
                reprs = []
                ft = self._factor_tensor.to(repr_model.device)
                for idx in dataloader_idx:  # TODO: batchfy when needed
            ...
            if learning_stage == "regime":
                self._regime_repr_tensor = (
                    torch.cat(reprs, dim=0).cpu().requires_grad_(False)
                )
            elif learning_stage == "signal":
                self._signal_repr_tensor = (
                    torch.cat(reprs, dim=0).cpu().requires_grad_(False)
                )
    ...
    def _create_regime_dataloader_idx(
        self,
        df: pd.DataFrame,
        train_date_range: tuple[str, str],
        valid_date_range: tuple[str, str],
        ...
        train_df = self._apply_date_range(df, train_date_range)
        valid_df = self._apply_date_range(df, valid_date_range)
        train_idx = train_df.groupby(DFKey.DATE).head(1).index
        valid_idx = valid_df.groupby(DFKey.DATE).head(1).index
        train_dataloader_idx = df.index.get_indexer(train_idx)
        valid_dataloader_idx = df.index.get_indexer(valid_idx)
        train_dataloader_idx = [
            idx
            for idx in train_dataloader_idx
            if idx > max(self._regime_lookback_num - 1, self._repr_lookback_num - 1)
        ]
        valid_dataloader_idx = [
            idx
            for idx in valid_dataloader_idx
            if idx > max(self._regime_lookback_num - 1, self._repr_lookback_num - 1)
        ]
        return train_dataloader_idx, valid_dataloader_idx
    ...
    def _load_data(
        ...
        resample_rule: str,
        gen_factorset: list[DictConfig],
    ) -> tuple[pd.DataFrame, pd.MultiIndex, pd.Series]:
        regime_label = pd.read_parquet(regime_label_fp).set_index("date")
        regime_label.loc[
            regime_label["distance"] > regime_distance_threshold, "label"
        ] = "others"
        ...
        return data, pd.MultiIndex.from_tuples(factor_cols), regime_label["label"]
    ...
    def _getitem_regime(
        ...
        repr_idx = self._regime_dataloader_idx_to_repr_idx[idx]

        repr_inputs = self._regime_repr_tensor[repr_idx]  # (24, 16)
        col_inputs = self._regime_col_tensor[
            idx - self._regime_lookback_num : idx
        ]  # (regime_lookback_num, col_num)

        dayofweek = self._df.iloc[idx].name.dayofweek
        gap = (
            self._df.iloc[idx][DFKey.FUTURE_PRICE_OPEN]
            - self._df.iloc[idx - 1][DFKey.FUTURE_PRICE_CLOSE]
        )
        days_to_expiration = calc_days_to_expiration_KOSPI200(
            self._df.iloc[idx].name.date()
        )
        meta_inputs = torch.tensor(
            [dayofweek, gap, days_to_expiration], dtype=torch.float32
        )

        label_name = self._regime_label[self._df.iloc[idx].name.date()]
        label = self._regime_label_map[label_name]

        timeindex = self._timeindex_tensor[idx]
        return repr_inputs, col_inputs, meta_inputs, label, timeindex

    def _getitem_signal(
        self, idx: int
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        repr_idx = self._signal_dataloader_idx_to_repr_idx[idx]
        inputs = self._signal_repr_tensor[repr_idx]
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, ...]:
        if self._learning_stage == "repr":
            ...
            return self._getitem_regime(idx)
