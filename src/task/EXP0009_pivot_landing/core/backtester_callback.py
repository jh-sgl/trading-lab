...
@register_callback("PivotLandingBacktester")
class PivotLandingBacktesterCallback(L.Callback):
    def __init__(
        self,
        ...
        super().__init__()
        self._backtester = PivotLandingBacktester(
            tick_slippage_size,
            commission_rate,
            price_multiplier,
            initial_balance,
    ...
    def on_validation_end(self, trainer: L.Trainer, pl_module) -> None:
        ...
        for (prefix, df), ignore_cost, psu in product(
            ...
            save_df = True if not ignore_cost and prefix == "TOTAL" else False
...
class PivotLandingBacktester:
    ...
    def _decide_position(self, df: pd.DataFrame, position_scaling_unit: float) -> pd.Series:
        output_pred = np.stack(df[DFKey.OUTPUT_PRED]) @ [-1, 0, 1]
        # pred_pivot_zone_idx = output_pred.argmax(axis=-1)
        # pred_pivot_zone_val = output_pred[np.arange(len(output_pred)), pred_pivot_zone_idx]

        # position = pd.Series(0, index=df.index)
        volatility = df[DFKey.VOLATILITY_50]
        # position_size = self._calc_max_position(volatility.clip(0.1, 10)) * pred_pivot_zone_val
        # idx_diff = df[DFKey.PRICE_PIVOT_ZONE] - pred_pivot_zone_idx
        # position[idx_diff < 0] = np.round(position_size * abs(idx_diff), 0)
        # position[idx_diff > 0] = np.round(-position_size * abs(idx_diff), 0)
        # position[pred_pivot_zone_val < position_scaling_unit] = 0
        # position[abs(idx_diff) == 0] = 0
