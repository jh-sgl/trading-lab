...
class TradingStats:
    ...
    def _calc_account(self, df: pd.DataFrame) -> pd.DataFrame:
        dailymax_balance = (
            ...
            .apply(
                lambda x: (x[DFKey.POSITION].cumsum() * x[DFKey.PRICE_EXECUTION] * Num.PRICE_MULTIPLIER / Num.UWON)
                .abs()
                .max()
            )
    ...
    def _calc_balance(self, df: pd.DataFrame, profit_uwon_key: str) -> tuple[pd.Series, pd.Series, pd.Series]:
        ...
        long_only_balance = df.loc[df[DFKey.POSITION] > 0, profit_uwon_key].cumsum()
        short_only_balance = df.loc[df[DFKey.POSITION] < 0, profit_uwon_key].cumsum()
    ...
    def calc_stats(self, df: pd.DataFrame, costless: bool) -> Stats:
        if costless:
            profit_uwon_key = DFKey.PROFIT_UWON_COSTLESS
            stat = self.stat_costless
        else:
            profit_uwon_key = DFKey.PROFIT_UWON
            stat = self.stat
...
class BacktesterCallback(L.Callback):
    def __init__(
        ...
        self._trading_stats = TradingStats(initial_balance)
    ...
    def on_validation_end(self, trainer: L.Trainer, pl_module) -> None:
        ...
        for prefix, df in dataset_info.items():
            self._run(trainer.log_dir, trainer.current_epoch, df, prefix, save_df=prefix == "TOTAL")
    ...
    def _run(
        ...
        df = self._backtester.run_backtest(df)
        stats = self._trading_stats.calc_stats(df, costless=False)
        stats_costless = self._trading_stats.calc_stats(df, costless=True)
        plot_save_fp, backtest_dataframe_fp = self._setup_save(log_dir, save_prefix, current_epoch, stats)

        plotter = ResultPlotter(stats, stats_costless)
...
class Backtester:
    ...
    def run_backtest(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        output_pred = np.stack(pred_df[DFKey.OUTPUT_PRED])

        b, t, c = output_pred.shape

        max_idx = np.argmax(output_pred.reshape(b, -1), axis=1)

        pred_df["time_idx"] = max_idx // c
        pred_df["class_idx"] = max_idx % c
        pred_df["max_vals"] = output_pred[np.arange(b), pred_df["time_idx"].values, pred_df["class_idx"].values]

        max_position = self._calc_max_position(volatility.clip(0.1, 10))
        pred_df[DFKey.POSITION] = 0.0
        pred_df.loc[pred_df["class_idx"] == 2, DFKey.POSITION] = max_position * pred_df["max_vals"]
        pred_df.loc[pred_df["class_idx"] == 0, DFKey.POSITION] = -max_position * pred_df["max_vals"]
        pred_df.loc[pred_df.index.hour >= self._trade_stop_hour, DFKey.POSITION] = 0

        def _get_row_profit(row):
            if row[DFKey.POSITION] < 0:  # sell
                return row[DFKey.SHORT_PROFIT_TRAJECTORY][row["time_idx"]]
            elif row[DFKey.POSITION] > 0:  # buy
                return row[DFKey.LONG_PROFIT_TRAJECTORY][row["time_idx"]]
            else:
                return 0.0

        def _get_row_profit_costless(row):
            if row[DFKey.POSITION] < 0:  # sell
                return row[DFKey.SHORT_PROFIT_TRAJECTORY_COSTLESS][row["time_idx"]]
            elif row[DFKey.POSITION] > 0:  # buy
                return row[DFKey.LONG_PROFIT_TRAJECTORY_COSTLESS][row["time_idx"]]
            else:
                return 0.0

        pred_df[DFKey.PROFIT] = pred_df.apply(_get_row_profit, axis=1) * Num.PRICE_MULTIPLIER
        pred_df[DFKey.PROFIT_COSTLESS] = pred_df.apply(_get_row_profit_costless, axis=1) * Num.PRICE_MULTIPLIER
        pred_df[DFKey.PROFIT_UWON] = pred_df[DFKey.PROFIT] / Num.UWON
        pred_df[DFKey.PROFIT_UWON_COSTLESS] = pred_df[DFKey.PROFIT_COSTLESS] / Num.UWON
        return pred_df


class ResultPlotter:
    def __init__(self, stats: TradingStats.Stats, stats_costless: TradingStats.Stats) -> None:
        # TODO: validate stats? with pydantic? or dataclasses?
        self._stats = stats
        self._stats_costless = stats_costless
    ...
    def _draw_balance_subplot(self, ax: Axes) -> None:
        ...
        ax.plot(
            self._stats_costless.balance.index,
            self._stats_costless.balance.values,
            linestyle="dotted",
            color="indianred",
            alpha=0.5,
            label="Balance (Costless)",
        )
        ax.plot(
            self._stats_costless.long_only_balance.index,
            self._stats_costless.long_only_balance.values,
            linestyle="dotted",
            color="lightsalmon",
            alpha=0.5,
            label="Long Only Balance (Costless)",
        )
        ax.plot(
            self._stats_costless.short_only_balance.index,
            self._stats_costless.short_only_balance.values,
            linestyle="dotted",
            color="cornflowerblue",
            alpha=0.5,
            label="Short Only Balance (Costless)",
        )
