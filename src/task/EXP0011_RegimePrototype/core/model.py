...
class BasicModel(L.LightningModule):
    def __init__(self, network: DictConfig, loss_func: DictConfig, lr: float) -> None:
        super().__init__()
    ...
    def _step(self, batch) -> tuple[torch.Tensor, torch.Tensor]:
        inputs_longterm, inputs_shortterm, label, _ = batch
        outputs = self._network(inputs_longterm, inputs_shortterm)
