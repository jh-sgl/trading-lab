...
from .model import RemainingPeakTroughPredictorModel
...
@register_callback("RemainingPeakTroughPredictorBacktester")
class RemainingPeakTroughPredictorBacktesterCallback(L.Callback):
    def __init__(
        self,
        ...
        market_closing_column: str,
    ) -> None:
        super().__init__()
        self._backtester = RemainingPeakTroughPredictorBacktester(
            tick_slippage_size,
            commission_rate,
            price_multiplier,
            initial_balance,
            ...
            trade_stop_hour,
            price_exit_column,
            market_closing_column,
        )
        self._position_scaling_unit = position_scaling_unit
        self._trading_stats = TradingStats(price_multiplier, initial_balance)

    def _create_output_df_from_cache(
        self,
        cache: CacheDict,
        keys: list[DataKey],
        price_move_mean: float,
        price_move_std: float,
    ) -> pd.DataFrame:
        output_df = pd.DataFrame({key: cache[key].cpu().numpy() for key in keys})

        output_df[DataKey.TIMESTAMP] = output_df[DataKey.TIMESTAMP].apply(lambda x: pd.to_datetime(x))
        output_df = output_df.set_index(DataKey.TIMESTAMP)
        output_df[DataKey.TIMESTAMP] = output_df.index

        output_df[DataKey.OUTPUT_PRED_BUY] = (
            output_df[DataKey.OUTPUT_PRED_BUY_NORMALIZED] * price_move_std
        ) + price_move_mean
        output_df[DataKey.OUTPUT_PRED_SELL] = (
            output_df[DataKey.OUTPUT_PRED_SELL_NORMALIZED] * price_move_std
        ) + price_move_mean

        return output_df

    def on_validation_end(self, trainer: L.Trainer, pl_module: RemainingPeakTroughPredictorModel) -> None:
        if trainer.sanity_checking:
            return

        keys = [DataKey.TIMESTAMP, DataKey.OUTPUT_PRED_BUY_NORMALIZED, DataKey.OUTPUT_PRED_SELL_NORMALIZED]

        train_output_df = self._create_output_df_from_cache(
            pl_module.train_cache,
            keys,
            trainer.datamodule.train_price_move_mean,
            trainer.datamodule.train_price_move_std,
        )
        val_output_df = self._create_output_df_from_cache(
            pl_module.val_cache,
            keys,
            trainer.datamodule.train_price_move_mean,
            trainer.datamodule.train_price_move_std,
        )
        total_output_df = pd.concat([train_output_df, val_output_df])

        train_dataset_df = trainer.datamodule.train_dataframe
        val_dataset_df = trainer.datamodule.val_dataframe
        total_dataset_df = pd.concat([train_dataset_df, val_dataset_df])

        dataset_info = {
            "TRAIN": (train_output_df, train_dataset_df),
            "VALID": (val_output_df, val_dataset_df),
            "TOTAL": (total_output_df, total_dataset_df),
        }

        ignore_cost_options = [False, True]

        for (prefix, (output_df, dataset_df)), ignore_cost, psu in product(
            dataset_info.items(), ignore_cost_options, self._position_scaling_unit
        ):
            prefix = f"{prefix}-COSTLESS" if ignore_cost else prefix
            self._run(trainer.log_dir, trainer.current_epoch, output_df, dataset_df, ignore_cost, prefix, psu)
    ...
    def _run(
        ...
        backtest_input_df = pd.merge(
            dataset_df, output_df, left_index=True, right_index=True, how="inner", validate="1:1"
        )

        backtest_result = self._backtester.run_backtest(
            backtest_input_df, position_scaling_unit, ignore_cost=ignore_cost
        )
        ...
        async def _ato_parquet(backtest_result, backtest_dataframe_fp):
            await asyncio.to_thread(backtest_result.to_parquet, backtest_dataframe_fp)

        asyncio.run(_ato_parquet(backtest_result, backtest_dataframe_fp))
        # backtest_result.to_parquet(backtest_dataframe_fp)


class RemainingPeakTroughPredictorBacktester:
    def __init__(
        self,
        tick_slippage_size: float,
        commission_rate: float,
        ...
        self._market_closing_column = market_closing_column
    ...
    def _decide_position(
        self,
        output_pred_buy: pd.Series,
        output_pred_sell: pd.Series,
        volatility: pd.Series,
        position_scaling_unit: float,
    ) -> pd.Series:

        # TODO: implement more delicate logic
        # output_pred = output_pred_buy + output_pred_sell
        output_pred = np.where(abs(output_pred_buy) > abs(output_pred_sell), output_pred_buy, output_pred_sell)
    ...
    def _calc_profit(self, df: pd.DataFrame, apply_price_multiplier: bool) -> pd.Series:
        price_move_list = []
        for date, daily_df in df.groupby(df.index.date, sort=False):
            fwd_max = daily_df[self._price_exit_column].shift(-1)[::-1].cummax()[::-1]
            fwd_min = daily_df[self._price_exit_column].shift(-1)[::-1].cummin()[::-1]
            pm = pd.Series(np.nan, index=daily_df.index)
            buy_exit = (daily_df["position"] > 0) & (
                fwd_max >= daily_df[DataKey.PRICE_ENTER] + daily_df[DataKey.OUTPUT_PRED_BUY]
            )
            sell_exit = (daily_df["position"] < 0) & (
                fwd_min <= daily_df[DataKey.PRICE_ENTER] + daily_df[DataKey.OUTPUT_PRED_SELL]
            )
            fallback = (daily_df["position"] != 0) & (~buy_exit & ~sell_exit)

            pm[buy_exit] = daily_df.loc[buy_exit, DataKey.OUTPUT_PRED_BUY]
            pm[sell_exit] = daily_df.loc[sell_exit, DataKey.OUTPUT_PRED_SELL]
            pm[fallback] = (
                daily_df.loc[fallback, self._market_closing_column] - daily_df.loc[fallback, DataKey.PRICE_ENTER]
            )

            price_move_list.append(pm)

        price_move = pd.concat(price_move_list).sort_index()

        profit = price_move * df["position"] - df["contract_cost"]
    ...
    def run_backtest(self, df: pd.DataFrame, position_scaling_unit: float, ignore_cost: bool = False) -> pd.DataFrame:
        df["position"] = self._decide_position(
            df[DataKey.OUTPUT_PRED_BUY_NORMALIZED],
            df[DataKey.OUTPUT_PRED_SELL_NORMALIZED],
            df[DataKey.PRICE_ENTER_VOLATILITY_50],
            position_scaling_unit,
        )
        df["contract_cost"] = self._calc_contract_cost(df["position"], df[DataKey.PRICE_ENTER], ignore_cost)
        df["profit"] = self._calc_profit(df, apply_price_multiplier=True)
        # TODO: use df directly
        backtest_result = self._create_result_dataframe(
            df[DataKey.TIMESTAMP], df["position"], df["contract_cost"], df[DataKey.PRICE_ENTER], df["profit"]
        )
