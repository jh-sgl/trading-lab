...
from .const import CacheDict, Key, Num
from .model import STReLUpstreamModel
...
class TradingStats:
    ...
    def _calc_account(self, df: pd.DataFrame) -> pd.DataFrame:
        dailymax_balance = (
            ...
            .apply(
                lambda x: (x[Key.POSITION].cumsum() * x[Key.PRICE_ENTER] * self._price_multiplier / Num.UWON)
                .abs()
                .max()
            )
        ...
        dailymax_position = (
            df.groupby(df.index.date)
            .apply(lambda x: (x[Key.POSITION].cumsum()).abs().max())
    ...
    def _calc_dailymax_position_signed(self, df: pd.DataFrame) -> pd.DataFrame:
        return df.groupby(df.index.date, group_keys=False).apply(
            lambda x: np.sign(x[Key.POSITION].cumsum().iloc[-1]) * x[Key.POSITION].cumsum().abs().max()
        )

    def _calc_balance(self, df: pd.DataFrame) -> tuple[pd.Series, pd.Series, pd.Series]:
        balance = df[Key.PROFIT_UWON].cumsum()
        long_only_balance = df.loc[df[Key.POSITION] > 0, Key.PROFIT_UWON].cumsum()
        short_only_balance = df.loc[df[Key.POSITION] < 0, Key.PROFIT_UWON].cumsum()
    ...
    def _calc_daily_profit_profile(self, df: pd.DataFrame) -> tuple[float, float, float, float]:
        daily_profit = df[Key.PROFIT_UWON].groupby(df.index.date).apply(lambda x: x.sum())
    ...
    def calc_stats(self, df: pd.DataFrame) -> Self:
        ...
        self.total_profit_uwon = df[Key.PROFIT_UWON].cumsum().iloc[-1]
...
@register_callback("STReLBacktester")
class STReLBacktesterCallback(L.Callback):
    def __init__(
        self,
        ...
        super().__init__()
        self._backtester = STReLBacktester(
            tick_slippage_size,
            commission_rate,
            price_multiplier,
            initial_balance,
    ...
    def on_validation_end(self, trainer: L.Trainer, pl_module: STReLUpstreamModel) -> None:
        if trainer.sanity_checking:
            return

        total_df = trainer.datamodule.val_df  # val_df includes all dates

        train_only_df = total_df.loc[trainer.datamodule.train_df.index].copy()
        ...
        for (prefix, df), ignore_cost, psu in product(
            ...
            save_df = False if ignore_cost else True
...
class STReLBacktester:
    ...
    def _decide_position(self, df: pd.DataFrame, position_scaling_unit: float) -> pd.Series:
        output_pred = df[Key.OUTPUT_PRED]
        volatility = df[Key.VOLATILITY_50]
    ...
    def _calc_profit(self, df: pd.DataFrame, apply_price_multiplier: bool) -> pd.Series:
        profit = df[Key.PRICE_MOVE] * df[Key.POSITION] - df[Key.COST]
    ...
    def run_backtest(self, df: pd.DataFrame, position_scaling_unit: float, ignore_cost: bool = False) -> pd.DataFrame:
        df[Key.POSITION] = self._decide_position(df, position_scaling_unit)
        df[Key.COST] = self._calc_contract_cost(df[Key.POSITION], df[Key.PRICE_ENTER], ignore_cost)
        df[Key.PROFIT] = self._calc_profit(df, apply_price_multiplier=True)
        df[Key.PROFIT_UWON] = df[Key.PROFIT] / Num.UWON
        return df
