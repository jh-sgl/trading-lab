    ...
    def on_validation_end(self, trainer: L.Trainer, pl_module) -> None:
        ...
        for prefix, date_range in date_ranges.items():
            ...
            if prefix == "TOTAL":
                ...
                total_twa_sharpe = stats.net_stat.twa_sharpe_ratio
                plotter.draw_result(
                    save_fp=plot_save_fp,
                    train_date_range=self._train_date_range,
                    valid_date_range=self._valid_date_range,
                    test_date_range=self._test_date_range,
                )
                if len(self._df_save_col) > 0:
                    df[self._df_save_col].to_parquet(backtest_dataframe_fp)
            elif prefix == "OOS":
                oos_sharpe = stats.net_stat.sharpe_ratio
                oos_twa_sharpe = stats.net_stat.twa_sharpe_ratio
            elif prefix == "TRAIN":
                train_sharpe = stats.net_stat.sharpe_ratio
                train_twa_sharpe = stats.net_stat.twa_sharpe_ratio
            elif prefix == "VALID":
                valid_sharpe = stats.net_stat.sharpe_ratio
                valid_twa_sharpe = stats.net_stat.twa_sharpe_ratio
            elif prefix == "TEST":
                test_sharpe = stats.net_stat.sharpe_ratio
                test_twa_sharpe = stats.net_stat.twa_sharpe_ratio

        shutil.move(
            f"{trainer.log_dir}/plot/E{trainer.current_epoch:04d}",
            f"{trainer.log_dir}/plot/E{trainer.current_epoch:04d}"
            f"_TR{train_sharpe:.3f}({train_twa_sharpe:.3f})"
            f"_VAL{valid_sharpe:.3f}({valid_twa_sharpe:.3f})"
            f"_TEST{test_sharpe:.3f}({test_twa_sharpe:.3f})"
            f"_OOS{oos_sharpe:.3f}({oos_twa_sharpe:.3f})"
            f"_TOT{total_sharpe:.3f}({total_twa_sharpe:.3f})",
        )
