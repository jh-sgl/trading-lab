#!/bin/bash
RUN_NAME_PREFIX="multirun_3_1y1y"
CONFIG_NAME="v1"
INPUT_DIM=16
...
run_job() {
  ...
  while (( attempt <= MAX_RETRIES )); do
    echo
    echo "🚴‍♀️ Running $ta_name (Attempt $attempt)"
    echo
    python -m task.EXP0016_SplitStrategy.train \
      --config-name=${CONFIG_NAME} \
      input_dim=${INPUT_DIM} \
      exp_name=${run_name}/${ta_name}/${valid_year} \
      ta_factorset_fp=${FACTORSET_DIR}${ta_name}.pt \
      train_date_range="[$train_year-01-01, $train_year-12-31]" \
...
for ta_name in "${TA_LIST[@]}"; do
  for ((i=0; i<${#YEAR_LIST[@]} - 2; i++)); do
    train_year="${YEAR_LIST[i]}"
    valid_year="${YEAR_LIST[i+1]}"
    test_year="${YEAR_LIST[i+2]}"

    run_job "$ta_name" "$RUN_NAME" "$train_year" "$valid_year" "$test_year" &
