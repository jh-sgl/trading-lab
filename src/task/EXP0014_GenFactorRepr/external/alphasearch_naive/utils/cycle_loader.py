...
class BalancedDataLoaderIterator:
    def __init__(self, dataloaders):
        ...
        print(
            "max dataloader length:",
            max_length,
            "epoch iteration:",
            max_length * self.num_dataloaders,
        )
        self.total_length = max_length * self.num_dataloaders
        self.current_iteration = 0
        self.probabilities = (
            torch.ones(self.num_dataloaders, dtype=torch.float) / self.num_dataloaders
        )
...
class CycleDataLoaderIterator:
    def __init__(self, dataloaders, max_iter=300):
        self.dataloaders = dataloaders
        self.num_dataloaders = len(dataloaders)
        ...
        self.iterators = [cycle(dataloader) for dataloader in self.dataloaders]
