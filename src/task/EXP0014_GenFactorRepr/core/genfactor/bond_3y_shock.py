...
@register_genfactor("bond_3y_shock")
class Bond3YShock(GenFactor):
    short_name = "bond_3y_shock"
    full_name = "3Y KTB Z-Score Shock"
    description = """
    ...
    def add_genfactor(self, df: pd.DataFrame) -> tuple[pd.DataFrame, tuple[str, str]]:
        sw = self.selected_params["shock_window"]
        log_ret = np.log(df[DFKey.BOND_165_CLOSE] + 1e-8).diff()
