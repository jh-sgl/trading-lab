#!/bin/bash
RUN_NAME_PREFIX="multirun_0b_lookbackall"
...
run_job() {
  ...
  while (( attempt <= MAX_RETRIES )); do
    ...
    echo
    python -m task.EXP0015_GenEventFilter.train \
      --config-name=v1 \
      exp_name=${run_name}/${exp_name} \
      ta_factorset_fp=/data/jh/repo/trading-lab/src/task/EXP0015_GenEventFilter/external/results/factorset/${exp_name}.pt \
      lookback_event_only=false
...
i=1
  ...
  for exp_name in "${EXPERIMENTS[@]}"; do
    RUN_NAME="${RUN_NAME_PREFIX}_${i}"  # lowercase genfactorset name
