...
class ResultPlotter:
    def __init__(
        self, stats: TradingStats, geneventfilter_name: str, factor_cols: pd.MultiIndex
    ) -> None:
        self._stats = stats
        self._geneventfilter_name = geneventfilter_name
    ...
    def _setup_initial_plot(self) -> tuple[Figure, Axes, Axes, Axes, Axes, Axes, Axes]:
        ...
        plt.suptitle(
            f"[{self._geneventfilter_name}] {str(timestamp.min())} ~ {str(timestamp.max())}",
            fontsize=16,
            y=0.98,
        )
    ...
    def _draw_factor_subplot(self, ax: Axes) -> None:
        factor_names = set([f"{lvl0}/{lvl1}" for lvl0, lvl1 in self._factor_cols])
    ...
    def draw_result(self, save_fp: str | None = None, show: bool = False) -> None:
