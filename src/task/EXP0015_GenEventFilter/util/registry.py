...
def register_geneventfilter(name):
    name = name.lower()

    def wrapper(cls):
        if name in GENEVENTFILTER_REGISTRY:
            raise ValueError(f"Duplicate geneventfilter registry key: {name}")
        GENEVENTFILTER_REGISTRY[name] = cls
        return cls
...
def build_geneventfilter(geneventfilter_cfg: DictConfig) -> "GenEventFilter":
    name = geneventfilter_cfg.name.lower()
    if geneventfilter_cfg.args == "random":
        return GENEVENTFILTER_REGISTRY[name](random_params=True)
    else:
        kwargs = {k: v for k, v in geneventfilter_cfg.args.items()}
        return GENEVENTFILTER_REGISTRY[name](**kwargs)
