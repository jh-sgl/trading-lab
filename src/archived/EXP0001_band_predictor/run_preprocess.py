from preprocess.after_merge_preprocessing_module import DailyShifter, DisparityExtractor
...
if __name__ == "__main__":
    ...
    after_merge_preprocessing_pipeline = [
        ...
        *[
            DisparityExtractor("future_price_close", f"future_price_close_disparity_{period}", period)
            for period in [5, 20, 100, 500, 2000]
        ],
        *[
            DisparityExtractor("usd_price_close", f"usd_price_close_disparity_{period}", period)
            for period in [5, 20, 100, 500, 2000]
        ],
    ]
    basic_preprocessor = BasicPreprocessor(
        ...
        ray_actor_num=48,
        save_fp="/data/jh/repo/trading-lab/data/preprocessed_jh/v4-5.parquet",
    )
