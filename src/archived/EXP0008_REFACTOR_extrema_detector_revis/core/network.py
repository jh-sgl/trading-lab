...
class LinearBatchRelu(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, lookback_num: int) -> None:
        ...
        self._norm = nn.InstanceNorm1d(lookback_num)
        self._relu = nn.GELU()
...
@register_network("ExtremaDetectorRevisNonlinear")
class ExtremaDetectorRevisNonlinear(nn.Module):
    def __init__(
        self,
        input_ch: int,
        hidden_dim: int,
...
@register_network("ExtremaDetectorRevisMLPDropout")
class ExtremaDetectorRevisMLPDropout(nn.Module):
    def __init__(self, input_ch, hidden_dim, num_outputs, lookback_num, drop_prob, *args, **kwargs):
        super().__init__()
        self._input_ch = input_ch
        self._num_outputs = num_outputs
        ...
        self.linear_1 = init_(torch.nn.Linear(input_ch * lookback_num, hidden_dim))
        self.linear_2 = init_(torch.nn.Linear(hidden_dim, self._num_outputs))
