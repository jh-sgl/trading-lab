...
class Basic(nn.Module):
    def __init__(
        ...
        # self._to_pred = nn.Linear(self._lookback_num // 16 * self._hidden_dim, 3)
        self._to_pred = nn.Linear(self._lookback_num // 8 * self._hidden_dim, 3)
    ...
    def _encode_inputs(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        encoded = self._encode_layer_1(encoded) + encoded
        encoded = self._lookback_layer_1(encoded.permute(0, 2, 1)).permute(0, 2, 1)
        encoded = self._encode_layer_2(encoded) + encoded
        encoded = self._lookback_layer_2(encoded.permute(0, 2, 1)).permute(0, 2, 1)
        encoded = self._encode_layer_3(encoded) + encoded
        encoded = self._lookback_layer_3(encoded.permute(0, 2, 1)).permute(0, 2, 1)
        # encoded = self._encode_layer_4(encoded) + encoded
        # encoded = self._lookback_layer_4(encoded.permute(0, 2, 1)).permute(0, 2, 1)
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        pred = self._to_pred(encoded.view(encoded.shape[0], -1)).squeeze(-1)
        return pred.softmax(-1)


class RegimeEncoder(nn.Module):
    def __init__(self, lookback_num: int, hidden_dim: int, regime_num: int) -> None:
        super().__init__()
        self._hidden_dim_layer_1 = LinearBatchRelu(hidden_dim, hidden_dim, lookback_num)
        self._lookback_layer_1 = LinearBatchRelu(lookback_num, lookback_num // 2, hidden_dim)
        self._hidden_dim_layer_2 = LinearBatchRelu(hidden_dim, hidden_dim, lookback_num // 2)
        self._lookback_layer_2 = LinearBatchRelu(lookback_num // 2, lookback_num // 4, hidden_dim)
        self._hidden_dim_layer_3 = LinearBatchRelu(hidden_dim, hidden_dim, lookback_num // 4)
        self._lookback_layer_3 = LinearBatchRelu(lookback_num // 4, lookback_num // 8, hidden_dim)

        self._to_regime_idx = nn.Linear(hidden_dim * lookback_num // 8, regime_num)
