...
class RawPreprocessingModuleBase(ABC):
    @abstractmethod
    def __call__(
        self, result_df: pd.DataFrame, *args: Any, **kwargs: Any
    ) -> pd.DataFrame:
        pass


class TimeConverter(RawPreprocessingModuleBase):
    def __init__(self, time_col_name: str, set_index_and_sort: bool = True) -> None:
    ...
    def __call__(self, result_df: pd.DataFrame) -> pd.DataFrame:
        ...
        result_df[self._time_col_name] = pd.to_datetime(
            result_df[self._time_col_name].astype(str), format="%H%M%S.%f"
        )
        result_df[self._time_col_name] = pd.to_timedelta(
            result_df[self._time_col_name].dt.strftime("%H:%M:%S.%f")
        )
        result_df[self._time_col_name] = date + result_df[self._time_col_name]

        if set_index_and_sort:
            result_df = result_df.set_index(self._time_col_name)
            result_df = result_df.sort_values(self._time_col_name)
        return result_df
...
class VolumeExtractor(RawPreprocessingModuleBase):
    ...
    def __call__(
        ...
        cumulative_volume = (
            result_df[self._source_long_col_name]
            + result_df[self._source_short_col_name]
        )
        result_df[self._target_cumulative_col_name] = cumulative_volume
        result_df[self._target_col_name] = cumulative_volume.diff().fillna(
            cumulative_volume.iloc[0]
        )
        return result_df
